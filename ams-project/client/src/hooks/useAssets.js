...
const INITIAL_FORM = {
  ...
  location_id:           "",
  department_id:         "",
  assigned_employee_id:  "",   // ★ optional at creation
  assigned_at:           "",   // ★ required only when employee selected
  purchase_date:         "",
  purchase_cost:         "",
  depreciation_method:   "",
  useful_life_years:     "",
...
function assetToForm(asset) {
  return {
    ...
    assigned_at:           asset.assigned_at
                             ? new Date(asset.assigned_at).toISOString().slice(0, 16)
                             : "",
...
export const useAssets = () => {
  ...
  useEffect(() => {
    ...
    if (form.department_id) {
      ...
      if (!valid && form.assigned_employee_id)
        setForm((prev) => ({ ...prev, assigned_employee_id: "", assigned_at: "" }));
    } else {
  ...
  const validateForm = () => {
    ...
    // ★ assigned_at is required only when an employee is selected
    if (form.assigned_employee_id && !form.assigned_at)
      errors.assigned_at = "Assignment date & time is required.";
  ...
  const buildPayload = () => ({
    ...
    assignedEmployeeId:  form.assigned_employee_id
                           ...
                           : null,
    assignedAt:          form.assigned_employee_id && form.assigned_at
                           ? new Date(form.assigned_at).toISOString()
                           : null,
  ...
  const openAddDrawer = () => {
    if (!canManage) { toast.error("You don't have permission to add assets."); return; }
  ...
  // NOTE: edit does NOT change assignment — use detail page modals for that
  const openEditDrawer = async (asset) => {
    if (!canManage) { toast.error("You don't have permission to edit assets."); return; }
  ...
  const openDeleteModal = (asset) => {
    if (!canAdmin) { toast.error("You don't have permission to delete assets."); return; }
  ...
  const closeViewModal  = () => { setViewModalOpen(false);   setSelectedAsset(null); };
  const closeDeleteModal = () => { setDeleteModalOpen(false); setSelectedAsset(null); };
  ...
  const handleSubmit = async () => {
    ...
    if (editingAsset) {
      ...
      if (res.success) { toast.success("Asset updated successfully."); closeDrawer(); fetchAssets(); }
      else toast.error(res.message);
    } else {
      const res = await createAssetApi(payload);
      if (res.success) { toast.success("Asset created successfully."); closeDrawer(); fetchAssets(); }
      else toast.error(res.message);
    }
  ...
  const handleToggleStatus = async (asset, newStatus) => {
    if (!canManage) { toast.error("You don't have permission to update asset status."); return; }
    const res = await updateAssetStatusApi(asset.id, newStatus);
    if (res.success) { toast.success(`Asset marked as ${newStatus}.`); fetchAssets(); }
    else toast.error(res.message);
  };
  ...
  const handleDelete = async (asset) => {
    ...
    if (res.success) { toast.success("Asset deleted successfully."); closeDeleteModal(); fetchAssets(); }
    else toast.error(res.message);
    setIsSubmitting(false);
  };
  ...
  return {
    assets, totalCount, loading, page, pageSize, setPage,
    ...
    locations, allDepartments, filteredDepartments,
    filteredEmployees,        // ★ passed to AssetForm
    ...
    handleToggleStatus, handleDelete, refresh,
    canAdmin, canManage,
  };
};