...
function Field({ label, required, error, hint, children }) {
  return (
    <div>
      ...
      <div className={`transition-all duration-200 overflow-hidden ${error ? "max-h-5 mt-1" : "max-h-0"}`}>
      ...
      {levels.map(({ depth, options, selectedId }) => {
        ...
        return (
          <div key={depth} className="relative">
            {depth > 0 && (
              <div className="absolute left-0 top-0 bottom-0 flex items-center" style={{ left: `${depth * 12}px` }}>
            ...
            <div style={{ paddingLeft: depth > 0 ? `${depth * 12 + 8}px` : 0 }}>
              <Field label={label} required={isFirst} error={fieldError}>
                <div className="relative">
                  ...
                  >
                    <option value="">{disabled ? "Loading…" : isFirst ? "— Select Category —" : `— Select ${label} —`}</option>
          ...
          <div className="flex items-center flex-wrap gap-1 pt-0.5">
            {crumbs.map((name, i) => (
              <span key={i} className="flex items-center gap-1">
                <span className="text-[11px] bg-red-50 text-red-700 border border-red-100 rounded-full px-2 py-0.5 font-medium">{name}</span>
                {i < crumbs.length - 1 && (
...
export default function AssetForm({
  ...
  const [openSections, setOpenSections] = useState({
    location: false, purchase: false, physical: false, insurance: false, amc: false,
  });
  ...
  function handleSubmit(e) {
    e.preventDefault();
    const locationErr  = !!(formErrors.location_id || formErrors.department_id || formErrors.assigned_at);
    ...
    if (locationErr || purchaseErr || physicalErr || insuranceErr) {
      setOpenSections({ location: locationErr, purchase: purchaseErr, physical: physicalErr, insurance: insuranceErr, amc: false });
    }
    onSubmit();
  }

  const locationHasError  = !!(formErrors.location_id || formErrors.department_id || formErrors.assigned_at);
  ...
  // Helper: default datetime to now (local)
  const localNow = () => {
    const d = new Date();
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 flex-shrink-0">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              {isEditing && editingAsset ? `Editing — ${editingAsset.asset_code}` : "Fill in the details below"}
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
          <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
            ...
            <Field label="Asset Name" required error={formErrors.asset_name}>
              <input
                type="text" name="asset_name" value={form.asset_name}
            ...
            <CategoryCascade
              ...
              onChange={onCategoryChange} error={formErrors.category_id}
              disabled={dropdownsLoading}
            />

            <Field label="Asset Type" required error={formErrors.asset_type}>
              <select name="asset_type" value={form.asset_type} onChange={handleChange} onBlur={handleBlur}
            ...
            <div className="grid grid-cols-2 gap-4">
              <Field label="Purchase Date" required error={formErrors.purchase_date}>
                <DatePicker value={form.purchase_date} onChange={(v) => handleDateChange("purchase_date", v)}
              ...
              <Field label="Purchase Cost (₹)" required error={formErrors.purchase_cost}>
                <input type="number" name="purchase_cost" min="0" step="0.01"
                  value={form.purchase_cost} onChange={handleChange} onBlur={handleBlur}
                  placeholder="0.00"
            ...
            <div className="grid grid-cols-2 gap-4">
              <Field label="Depreciation Method" required error={formErrors.depreciation_method}
                hint="SLM = fixed ₹/yr  ·  WDV = % of book value">
                <select name="depreciation_method" value={form.depreciation_method}
                  onChange={handleChange} onBlur={handleBlur}
              ...
              <Field label="Useful Life (Years)" required error={formErrors.useful_life_years} hint="e.g. 3, 5, 10">
                <input type="number" name="useful_life_years" min="1" max="99" step="1"
                  value={form.useful_life_years} onChange={handleChange} onBlur={handleBlur}
                  placeholder="e.g. 5"
            ...
            >
              <Field label="Location" required error={formErrors.location_id}>
                <select name="location_id" value={form.location_id}
                  onChange={handleChange} onBlur={handleBlur} disabled={dropdownsLoading}
                ...
                <select name="department_id" value={form.department_id}
                  ...
                  <option value="">
                    {!form.location_id ? "Select location first"
                      : filteredDepartments.length === 0 ? "No departments for this location"
                      : "— Select Department —"}
                  </option>
              ...
              {/* ★ Assign Employee — optional, only on CREATE */}
              {!isEditing && (
                <>
                  <Field label="Assign Employee" hint="Optional — can assign later from asset detail page">
                    <select
                      name="assigned_employee_id"
                      value={form.assigned_employee_id}
                      onChange={handleChange}
                      onBlur={handleBlur}
                      disabled={!form.department_id || dropdownsLoading}
                      className={`${inputClass("", false, form.assigned_employee_id)} disabled:opacity-50 disabled:cursor-not-allowed`}
                    >
                      <option value="">
                        {!form.department_id ? "Select department first" : "— Unassigned (optional) —"}
                      </option>
                      {employees.map((e) => (
                  ...
                  {/* ★ Assignment date — only shows when employee is selected */}
                  {form.assigned_employee_id && (
                    <Field label="Assignment Date & Time" required error={formErrors.assigned_at}>
                      <input
                        type="datetime-local"
                        name="assigned_at"
                        ...
                        onChange={handleChange}
                        onBlur={handleBlur}
                        onFocus={(e) => { if (!e.target.value) onFormChange("assigned_at", localNow()); }}
                        className={inputClass(formErrors.assigned_at, touched.assigned_at, form.assigned_at)}
                      />
                    </Field>
                  )}
                </>
              ...
              {/* On edit — remind user assignment is managed via detail page */}
              {isEditing && (
                <div className="rounded-lg bg-blue-50 border border-blue-100 px-4 py-3 text-xs text-blue-700">
                  <strong>Assignment</strong> is managed from the Asset Detail page — use the
                  Assign / Collect / Reassign buttons there.
                </div>
              )}
            </CollapsibleSection>

            {/* ── Purchase Details ── */}
            <CollapsibleSection title="Purchase Details" isOpen={openSections.purchase}
              onToggle={() => toggleSection("purchase")} hasError={purchaseHasError}>
              <div className="grid grid-cols-2 gap-4">
                <Field label="Scrap Value (₹)" error={formErrors.scrap_value} hint="Optional">
                  <input type="number" name="scrap_value" min="0" step="0.01"
                    value={form.scrap_value} onChange={handleChange} onBlur={handleBlur}
                    placeholder="0.00"
                ...
                <Field label="Vendor" error={formErrors.vendor}>
                  <input type="text" name="vendor" value={form.vendor}
                    onChange={handleChange} onBlur={handleBlur} placeholder="e.g. Dell India"
              ...
              <div className="grid grid-cols-2 gap-4">
                <Field label="Invoice Number" error={formErrors.invoice_number}>
                  <input type="text" name="invoice_number" value={form.invoice_number}
                    onChange={handleChange} onBlur={handleBlur} placeholder="INV-00001"
                ...
                <Field label="Invoice Date" error={formErrors.invoice_date}>
                  <DatePicker value={form.invoice_date} onChange={(v) => handleDateChange("invoice_date", v)}
                    placeholder="Select date"
                    triggerStyle={dpTrigger(formErrors.invoice_date, touched.invoice_date, form.invoice_date)}
                    containerStyle={dpContainer} alignRight popupFixed />
                </Field>
              </div>
              <Field label="Warranty Expiry Date" error={formErrors.warranty_expiry} hint="Optional — as per invoice / box">
                <DatePicker value={form.warranty_expiry} onChange={(v) => handleDateChange("warranty_expiry", v)}
                  placeholder="Select date"
            ...
            {/* ── Physical Details ── */}
            <CollapsibleSection title="Physical Details" isOpen={openSections.physical}
              onToggle={() => toggleSection("physical")} hasError={physicalHasError}>
              <div className="grid grid-cols-2 gap-4">
                <Field label="Serial Number" error={formErrors.serial_number}>
                  <input type="text" name="serial_number" value={form.serial_number}
                    onChange={handleChange} onBlur={handleBlur} placeholder="SN-XXXXXXXX"
                ...
                <Field label="Model Number" error={formErrors.model_number}>
                  <input type="text" name="model_number" value={form.model_number}
                    onChange={handleChange} onBlur={handleBlur} placeholder="e.g. LAT-5510"
              ...
              <div className="grid grid-cols-2 gap-4">
                <Field label="Brand" error={formErrors.brand}>
                  <input type="text" name="brand" value={form.brand}
                    onChange={handleChange} onBlur={handleBlur} placeholder="e.g. Dell"
                ...
                <Field label="Color" error={formErrors.color}>
                  <input type="text" name="color" value={form.color}
                    onChange={handleChange} onBlur={handleBlur} placeholder="e.g. Black"
            ...
            {/* ── Insurance ── */}
            <CollapsibleSection title="Insurance (Optional)" isOpen={openSections.insurance}
              onToggle={() => toggleSection("insurance")} hasError={insuranceHasError}>
              <Field label="Policy Number" error={formErrors.insurance_policy_no}>
                <input type="text" name="insurance_policy_no" value={form.insurance_policy_no}
                  onChange={handleChange} onBlur={handleBlur} placeholder="POL-XXXXXXXX"
              ...
              <div className="grid grid-cols-2 gap-4">
                <Field label="Insurance Company" error={formErrors.insurance_company}>
                  <input type="text" name="insurance_company" value={form.insurance_company}
                    onChange={handleChange} onBlur={handleBlur} placeholder="e.g. HDFC Ergo"
                ...
                <Field label="Expiry Date" error={formErrors.insurance_expiry_date}>
                  <DatePicker value={form.insurance_expiry_date} onChange={(v) => handleDateChange("insurance_expiry_date", v)}
                    placeholder="Select date"
            ...
            {/* ── AMC ── */}
            <CollapsibleSection title="AMC (Optional)" isOpen={openSections.amc}
              onToggle={() => toggleSection("amc")} hasError={false}>
              <div className="grid grid-cols-2 gap-4">
                <Field label="AMC Vendor" error={formErrors.amc_vendor}>
                  <input type="text" name="amc_vendor" value={form.amc_vendor}
                    onChange={handleChange} onBlur={handleBlur} placeholder="e.g. HP Service Center"
                ...
                <Field label="AMC Expiry Date" error={formErrors.amc_expiry_date}>
                  <DatePicker value={form.amc_expiry_date} onChange={(v) => handleDateChange("amc_expiry_date", v)}
                    placeholder="Select date"
          ...
          <div className="px-6 py-4 border-t border-gray-100 flex gap-3 flex-shrink-0">
            ...
            <button type="submit" disabled={isSubmitting || dropdownsLoading}
